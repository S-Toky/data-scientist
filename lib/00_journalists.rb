...
def aude
	count = 0
	data.each do |email|
  		if email.match(/[a||A][u||U][d||D][e||E]/)
      		count+=1
  		end
	end
	return count
...
def Affiche_Resultat
	puts "Le nombre de pseudo #{data.length}"
	puts "Le nombre de psedo qui contient de chiffre #{compter}"
